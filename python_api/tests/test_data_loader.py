...
from python_api.src.data_loader import (
    load_tracking_data,
    load_event_data,
...
def test_load_tracking_data_success(mock_read_parquet, caplog_fixture):
    dummy_df_content = {
        'timestamp_ms': [0, 100], 'x': [1, 2], 'y': [3, 4],
    ...
    for col in EXPECTED_TRACKING_COLS:
        if col not in dummy_df_content:
            dummy_df_content[col] = [None] * len(dummy_df_content['timestamp_ms']) # Add dummy data or appropriate type
...
def test_load_tracking_data_missing_essential_cols(mock_read_parquet, caplog_fixture):
    ...
    assert f"Essential columns missing in tracking data: {file_path}" in caplog_fixture.text
...
def test_load_tracking_data_generic_exception(mock_read_parquet, caplog_fixture):
    ...
    assert result_df.empty
    assert sorted(list(result_df.columns)) == sorted(EXPECTED_TRACKING_COLS)
    assert f"Error loading tracking data from {file_path}: Some generic Parquet error" in caplog_fixture.text
...
def test_load_tracking_data_path_conversion():
    ...
    for col in EXPECTED_TRACKING_COLS: # Ensure some dummy data for essential check if any
        if col in ['timestamp_ms', 'x', 'y', 'player_id', 'team_id', 'smooth_x_speed', 'smooth_y_speed']:
...
def test_load_event_data_success(mock_read_parquet, caplog_fixture):
    dummy_df_content = {
        'event_id': [1, 2], 'event_type': ['PASS', 'SHOT'], 'timestamp_ms': [50, 90],
        'player_id': ['p1', 'p1'], 'team_id': ['tA', 'tA'],
...
def test_load_event_data_missing_essential_cols(mock_read_parquet, caplog_fixture):
    ...
    assert f"Essential columns missing in event data: {file_path}" in caplog_fixture.text
...
def test_load_event_data_generic_exception(mock_read_parquet, caplog_fixture):
    ...
    assert result_df.empty
    assert sorted(list(result_df.columns)) == sorted(EXPECTED_EVENT_COLS)
    assert f"Error loading event data from {file_path}: Some generic Parquet error for event" in caplog_fixture.text
...
def test_load_event_data_path_conversion():
    ...
    for col in EXPECTED_EVENT_COLS:
         if col in ['event_id', 'event_type', 'timestamp_ms', 'player_id', 'team_id', 'start_x', 'start_y', 'end_x', 'end_y']:
             mock_df_with_all_cols[col] = [0] # Minimal data to pass checks
         else:
             mock_df_with_all_cols[col] = [None]
