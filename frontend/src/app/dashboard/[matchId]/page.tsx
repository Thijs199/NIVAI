...
export default function MatchDashboardPage() {
  ...
  const [playerTimeSeries, setPlayerTimeSeries] = useState<PlayerTimeSeriesDataPoint[] | null>(
    null
  );
  ...
  useEffect(() => {
    if (matchId) {
      ...
      fetch(`/api/v1/analytics/matches/${matchId}`)
        .then((res) => {
          if (!res.ok)
            throw new Error(`Failed to fetch match summary: ${res.status} ${res.statusText}`);
          return res.json();
        })
    ...
    if (selectedPlayerId && matchId) {
      ...
      fetch(`/api/v1/analytics/players/${selectedPlayerId}/details?match_id=${matchId}`)
        ...
        .then((data: { time_series: PlayerTimeSeriesDataPoint[] }) => {
          const processedData = data.time_series.map((d) => ({
            ...d,
            time_s: d.timestamp_ms / 1000,
          }));
        ...
        .catch((err) => {
          ...
          setError((prev) =>
            prev
              ? `${prev}\nFailed to load player time series: ${err.message}`
              : `Failed to load player time series: ${err.message}`
          );
        })
  ...
  useEffect(() => {
    if (selectedTeamIdForChart && matchId) {
      ...
      fetch(
        `/api/v1/analytics/teams/${selectedTeamIdForChart}/summary-over-time?match_id=${matchId}`
      )
        .then((res) => {
        ...
        .catch((err) => {
          ...
          setError((prev) =>
            prev
              ? `${prev}\nFailed to load team interval data: ${err.message}`
              : `Failed to load team interval data: ${err.message}`
          );
        })
  ...
  if (error && !summaryData && !isLoadingSummary)
    return <div className="p-4 text-center text-red-500">Error loading match data: {error}</div>;
  if (!summaryData)
    return <div className="p-4 text-center">No summary data available for this match.</div>;

  const selectedPlayerData = selectedPlayerId
    ? allPlayersList.find((p) => p.player_id === selectedPlayerId)
    : null;
  ...
  return (
    <div className="container mx-auto p-4 space-y-8">
      <h1 className="text-3xl font-bold text-center text-slate-800">
        Match Dashboard: {summaryData.match_id || matchId}
      </h1>
      {error && (
        <div
          className="p-4 my-4 bg-red-100 text-red-700 border-l-4 border-red-500 rounded-md text-sm"
          role="alert"
        >
      ...
      <section className="p-6 bg-white shadow-lg rounded-lg">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            <h3 className="text-xl font-medium mb-2 text-slate-600">
              Total Distance Run by Player (km)
            </h3>
            <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="mt-6">
          ...
          <select
            onChange={(e: ChangeEvent<HTMLSelectElement>) =>
              setSelectedTeamIdForChart(e.target.value)
            }
          ...
          {teamIntervalData && teamIntervalData.length > 0 && (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart
                data={teamIntervalData.map((d) => ({
                  ...d,
                  interval: `${(d.interval_start_time_s / 60).toFixed(0)}-${(d.interval_end_time_s / 60).toFixed(0)}min`,
                }))}
              >
                <CartesianGrid strokeDasharray="3 3" />
                ...
                <YAxis
                  yAxisId="left"
                  label={{ value: 'Avg Speed (km/h)', angle: -90, position: 'insideLeft' }}
                />
                <YAxis
                  yAxisId="right"
                  orientation="right"
          ...
          {!teamIntervalData && !isLoadingTeamInterval && selectedTeamIdForChart && (
            <p className="p-2">
              No interval data available for{' '}
              {summaryData.teams[selectedTeamIdForChart]?.team_name || selectedTeamIdForChart}.
            </p>
          )}
        </div>
      </section>
      ...
      <section className="p-6 bg-white shadow-lg rounded-lg">
        ...
        {selectedPlayerId && selectedPlayerData && (
          <div className="grid md:grid-cols-3 gap-6 items-start">
            <div className="md:col-span-1 p-4 border rounded bg-slate-50 shadow-sm">
              <h3 className="text-xl font-medium mb-2 text-slate-600">
                {selectedPlayerData.player_name || selectedPlayerData.player_id}
              </h3>
              <div className="player-image-section mb-4">
                {isLoadingImage && (
                  <div className="w-32 h-32 bg-gray-200 flex items-center justify-center text-gray-500 rounded-md border animate-pulse">
                    Loading Image...
                  </div>
                )}
                {imageError && (
                  <div className="w-32 h-32 bg-red-100 flex items-center justify-center text-red-500 rounded-md border p-2">
                    {imageError}
                  </div>
                )}
                {!isLoadingImage && !imageError && playerImageUrl && (
                  <Image
                    src={playerImageUrl}
                    alt={`Profile of ${selectedPlayerData.player_name || selectedPlayerData.player_id}`}
                    width={128}
                    height={128}
                    className="object-cover rounded-md border shadow"
                  />
                )}
                {!isLoadingImage && !imageError && !playerImageUrl && (
              ...
              <p>
                <strong>Team:</strong> {selectedPlayerData.team_id}
              </p>
              <p>
                <strong>Total Distance:</strong>{' '}
                {(selectedPlayerData.total_distance_m / 1000).toFixed(2)} km
              </p>
              {selectedPlayerData.total_sprint_distance_m !== undefined && (
                <p>
                  <strong>Sprint Distance:</strong>{' '}
                  {(selectedPlayerData.total_sprint_distance_m / 1000).toFixed(2)} km
                </p>
              )}
              {selectedPlayerData.num_accelerations !== undefined && (
                <p>
                  <strong>Accelerations:</strong> {selectedPlayerData.num_accelerations}
                </p>
              )}
              {selectedPlayerData.num_decelerations !== undefined && (
                <p>
                  <strong>Decelerations:</strong> {selectedPlayerData.num_decelerations}
                </p>
              )}
              {selectedPlayerData.max_speed_kmh !== undefined && (
                <p>
                  <strong>Max Speed:</strong> {selectedPlayerData.max_speed_kmh.toFixed(2)} km/h
                </p>
              )}
              {selectedPlayerData.avg_speed_kmh !== undefined && (
                <p>
                  <strong>Average Speed:</strong> {selectedPlayerData.avg_speed_kmh.toFixed(2)} km/h
                </p>
              )}
              <p className="text-xs italic text-gray-500 mt-4">
                Note: Detailed pass statistics and EPV performance data are not yet available.
              </p>
            </div>
            <div className="md:col-span-2">
              ...
              {playerTimeSeries && playerTimeSeries.length > 0 ? (
                  ...
                  <LineChart data={playerTimeSeries}>
                    ...
                    <Tooltip
                      formatter={(value: number, name: string) =>
                        name === 'Speed' ? `${value.toFixed(2)} km/h` : value
                      }
                    />
                    <Legend />
                    <Line
                ...
                <p>
                  {!isLoadingPlayer && selectedPlayerId
                    ? 'No time series data available for this player.'
                    : ''}
                </p>
              )}
            </div>
        ...
        {!selectedPlayerId && (
          <p className="text-center text-slate-500 py-4">Select a player to view details.</p>
        )}
      </section>
    </div>
  );
